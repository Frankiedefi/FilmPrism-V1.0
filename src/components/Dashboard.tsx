...
import { Film, FileText, DollarSign, Calendar, Users, LogOut, Sparkles, ChevronLeft, ChevronRight } from 'lucide-react';
...
function Dashboard() {
  ...
  return (
    <div className="flex h-screen bg-gray-100">
      {/* Sidebar */}
      <div className={`w-${isSidebarMinimized ? '16' : '64'} bg-indigo-800 text-white transition-all duration-300`}>
        <div className="flex items-center justify-center h-16 bg-indigo-900">
          <Film className="h-8 w-8 mr-2" />
          {!isSidebarMinimized && <span className="text-xl font-semibold">FilmPro AI</span>}
        </div>
        <nav className="mt-8">
          <button onClick={toggleSidebar} className="flex items-center px-4 py-2 hover:bg-indigo-700">
            {isSidebarMinimized ? <ChevronRight className="h-5 w-5" /> : <ChevronLeft className="h-5 w-5" />}
          </button>
          {!isSidebarMinimized && (
            <>
